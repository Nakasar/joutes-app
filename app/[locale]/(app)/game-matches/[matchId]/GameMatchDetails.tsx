...
import RatingSelector from "./RatingSelector.tsx";
import { MAX_GUEST_NAME_LENGTH, guestId } from "@/lib/matches/participants.ts";
import { nanoid } from "nanoid";
import DeckSelector from "@/components/DeckSelector.tsx";
import BattleReportSection from "./BattleReportSection.tsx";
...
export default function GameMatchDetails({
  ...
  useEffect(() => {
    const loadDecks = async () => {
      if (!match.decks || Object.keys(match.decks).length === 0) return;
      
      const deckIds = Object.values(match.decks);
      const decksData: Record<string, Deck> = {};
                  ...
                  >
                    ...
                    {playerDeck && (
                      <div className="text-xs text-muted-foreground pl-2 border-l-2 border-muted-foreground/30">
                        Deck : <span className="font-medium text-foreground">{playerDeck.name}</span>
                      </div>
                    )}
                  </div>
                );
