...
import { TournamentLinkCard } from "./TournamentLinkCard.tsx";
...
/**
 * Deux frontières : l'en-tête ne tient qu'à l'événement — nom, jeu, état — quand
 * le corps demande en plus la session, le tournoi rattaché, et pour
 * l'organisation seule la liste des participants.
 *
 * Les bandeaux d'inscription et d'erreur ne tiennent qu'à la query : ils
 * arrivent avec l'en-tête.
 */
...
async function EventHeader({ params, searchParams }: EventPageProps) {
  const { eventId } = await params;
  const [search, { event, isPrivateEvent, hasAccess }, t] = await Promise.all([
    searchParams,
...
async function EventBody({ params }: Pick<EventPageProps, "params">) {
  const { eventId } = await params;
  const [{ event, isPrivateEvent, hasAccess }, t, tTournaments, locale] = await Promise.all([
    readEventAccess(eventId),
    getTranslations("EventDetail"),
  ...
  const isCreator = Boolean(session?.user && event.creatorId === session.user.id);
  const isOrganizerStaff = Boolean(session?.user && event.staff?.some(
