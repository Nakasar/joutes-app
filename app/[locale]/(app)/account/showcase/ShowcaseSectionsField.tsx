...
import type { UserShowcaseSection, UserShowcaseSectionKey } from "@/lib/users/showcase.ts";
import { cn } from "@/lib/utils.ts";

/**
 * L'ordre et l'activation des blocs de la vitrine.
 *
 ...
 *
 * Le verrou porte sur l'activation, pas sur la position : « souhaits et
 * ventes » garde ses flèches, seul son interrupteur disparaît — sa visibilité
 * se règle liste par liste, sur chaque liste.
 */
export default function ShowcaseSectionsField({
  sections,
  onChange,
  disabled,
}: {
  sections: UserShowcaseSection[];
  onChange: (sections: UserShowcaseSection[]) => void;
  disabled?: boolean;
}) {
  const t = useTranslations("Account.showcase.sections");
  const [dragged, setDragged] = useState<UserShowcaseSectionKey | null>(null);
      ...
      {sections.map((section, index) => (
        ...
        >
          ...
          <span className="min-w-0 flex-1 text-sm">
            {t(`labels.${section.key}` as "labels.about")}
          </span>
          ...
          <div className="flex shrink-0 items-center">
            <button
              ...
              aria-label={t("moveUp", { section: t(`labels.${section.key}` as "labels.about") })}
            ...
            <button
              ...
              onClick={() => move(index, index + 1)}
              aria-label={t("moveDown", { section: t(`labels.${section.key}` as "labels.about") })}
              className="rounded p-1 text-muted-foreground transition-colors hover:text-foreground disabled:opacity-30"
            >
          ...
          {!section.locked && (
            <Switch
              ...
              disabled={disabled}
              aria-label={t(`labels.${section.key}` as "labels.about")}
