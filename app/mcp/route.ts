...
async function handleSearchEvents(args: any, userId?: string): Promise<MCPResponse> {
  try {
    ...
    // Filtrer par jeux si spécifié
    if (args.gameNames && Array.isArray(args.gameNames)) {
      events = events.filter(event => 
        args.gameNames.some((gameName: string) => 
          event.gameName.toLowerCase().includes(gameName.toLowerCase())
        )
      );
    }
    ...
    if (args.lairName) {
      events = events.filter(event => 
        event.lair?.name.toLowerCase().includes(args.lairName.toLowerCase())
      );
    }
