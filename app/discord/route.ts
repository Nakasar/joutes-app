      ...
      body: {
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(`Voici les évènements à venir : ${events.length > 0 ? events.map(e => `-${e.game?.slug ? `:${e.game.slug}:` : ''} [${e.name}](https://joutes.app/events/${e.id}) le ${DateTime.fromISO(e.startDateTime, { locale: 'Europe/Paris'}).toLocaleString(DateTime.DATETIME_MED)}`).join('\n') : 'Aucun évènement à venir.'}`)
        ],
        content: null,
      },
    },
