...
export default function GamesExplorer({ games }: GamesExplorerProps) {
  ...
  const gamesByType = useMemo(() => {
    ...
    filteredGames.forEach((game) => {
      const typeLabel = GAME_TYPES[game.type] || t("explorer.categories.other");
        ...
        <div className="relative z-20 w-full max-w-4xl px-4 space-y-6 text-center">
          ...
          {searchQuery && (
            <p className="text-gray-400 animate-fade-in">
              {filteredGames.length > 1 ? t("explorer.resultsPlural", { count: filteredGames.length }) : t("explorer.resultsSingular", { count: filteredGames.length })}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="relative z-20 px-4 md:px-8 pb-20 -mt-20">
        {Object.entries(gamesByType).length === 0 ? (
          <div className="text-center py-20">
            <p className="text-2xl text-gray-400">{t("explorer.noGames")}</p>
            <p className="text-gray-500 mt-2">{t("explorer.noGamesHint")}</p>
          </div>
        ) : (
