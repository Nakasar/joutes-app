...
type Params = { params: Promise<{ tournamentId: string; playerId: string; awardId: string }> };
...
export async function DELETE(request: NextRequest, { params }: Params) {
  ...
  try {
    const { tournamentId, playerId, awardId } = await params;
    const tournament = await requireTournament(tournamentId);
    assertCanManage(tournament, user.userId);

    await deleteFeatAward(tournamentId, awardId, playerId);
