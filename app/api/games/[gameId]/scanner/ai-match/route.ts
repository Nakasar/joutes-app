...
export async function POST(request: Request, { params }: { params: Promise<{ gameId: string }> }) {
  ...
  const exactMatch = await findCardByAiIdentification(new ObjectId(game.id), {
  ...
  const match = exactMatch
    ? { id: exactMatch.id, name: exactMatch.name, score: 1 }
    : identification.cardName
      ? await matchCardNameInMeilisearch(game.slug, identification.cardName, {
          lang: identification.lang,
          setCode: identification.setCode,
        })
      : null;
