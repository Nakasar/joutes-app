...
describe("matchCardnexusProducts", () => {
  it("rapproche une carte par son extension et son numéro", () => {
    const { matches } = matchCardnexusProducts(
      [product(1, 42, "027")],
      [expansion(42, "OGN")],
      [card("OGN027", "OGN", "027")]
    );
  ...
  it("rattache tous les produits d'un même numéro à la même carte", () => {
    const { matches } = matchCardnexusProducts(
  ...
  it("distingue une variante de son numéro de base", () => {
    const { matches } = matchCardnexusProducts(
  ...
  it("n'attribue rien quand deux cartes partagent le même numéro", () => {
    const { matches, skipped } = matchCardnexusProducts(
      [product(1, 42, "5")],
      [expansion(42, "SOR")],
  ...
  it("écarte une extension dont CardNexus ne publie pas le code", () => {
    const { matches, skipped } = matchCardnexusProducts(
      [product(1, 42, "027")],
      [expansion(42, null)],
      [card("OGN027", "OGN", "027")]
  ...
  it("retrouve par son slug une extension sans code, quand le profil le dit", () => {
    const { matches } = matchCardnexusProducts(
      [product(1, 42, "027")],
  ...
  it("traduit un code d'extension qui s'écrit autrement chez nous", () => {
    const { matches } = matchCardnexusProducts(
      [product(1, 42, "027")],
  ...
  it("compte, extension par extension, ce qui a été rapproché", () => {
    const { expansions } = matchCardnexusProducts(
  ...
  it("écarte un produit sans numéro de collection", () => {
    const { matches, skipped } = matchCardnexusProducts(
      [product(1, 42, null)],
      [expansion(42, "OGN")],
      [card("OGN027", "OGN", "027")]
