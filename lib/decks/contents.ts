import {
  ...
  type DeckZone,
  type DeckZoneKey,
} from "@/lib/decks/zones";

/**
 * Une carte du deck, dans une zone : l'identifiant du catalogue et le nombre
 * d'exemplaires. Le nom n'est pas recopié — il vient du catalogue, qui est la
 * seule source à jour d'une carte (nom traduit, illustration, errata).
 */
 ...
 * Un sous-ensemble du document de catalogue, rempli par
 * `lib/db/deck-cards.ts` côté serveur et par la recherche côté éditeur.
 */
export type DeckCardInfo = {
  ...
  /** Coût de jeu de la carte ; les cartes sans coût (légendes, runes) n'en ont pas. */
  cost?: number;
  domain?: string[];
...
/**
 * État de chaque zone au regard de sa contrainte de taille.
 *
 * C'est de là que sortent le badge de la fiche, celui des en-têtes de zone et
 * le panneau « Légalité » de l'éditeur : les trois disent la même chose parce
 * qu'ils lisent le même calcul, jamais une valeur écrite à la main.
 */
...
/** Répartition du deck par type de carte, du type le plus représenté au moins représenté. */
...
export type CollectionCoverage = {
  /** Exemplaires du deck déjà couverts par la collection. */
  owned: number;
  /** Exemplaires qu'il reste à trouver. */
  missing: number;
  /** Identifiants des cartes dont il manque au moins un exemplaire. */
  missingCardIds: string[];
};

/**
 * Ce que la collection couvre du deck.
 *
 * Le besoin se cumule par carte avant d'être comparé : la même carte jouée dans
 * le deck principal et dans la réserve ne se dédouble pas, on ne possède pas
 * deux fois le même exemplaire.
 *
 * Le calcul vit ici plutôt que côté serveur pour que l'éditeur le refasse à
 * chaque changement : un panneau qui annoncerait « 51 possédées » sur un deck
 * dont on vient de retirer dix cartes commenterait un deck qui n'existe plus.
 */
export function collectionCoverage(
  cards: DeckCards | undefined,
  zones: DeckZone[],
  ownedByCardId: Map<string, number> | undefined
): CollectionCoverage {
  const needed = new Map<string, number>();

  for (const zone of zones) {
    for (const entry of zoneEntries(cards, zone.key)) {
      needed.set(entry.cardId, (needed.get(entry.cardId) ?? 0) + entry.quantity);
    }
  }
  ...
  for (const [cardId, quantity] of needed) {
    const have = Math.min(ownedByCardId?.get(cardId) ?? 0, quantity);
    owned += have;
    if (have < quantity) {
  ...
  return { owned, missing, missingCardIds };
}

/**
 * Nombre maximum d'exemplaires d'une même carte, toutes zones confondues.
 *
 * Sert au contrôle « Max exemplaires » du panneau de légalité. Les zones à
 * cible exacte d'une carte unique (la légende) en sont exclues : elles ont déjà
 * leur propre règle, et une rune jouée douze fois n'est pas une infraction.
 */
